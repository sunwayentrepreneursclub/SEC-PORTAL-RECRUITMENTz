...
| `ADMIN_USERS` | `amadeus:somepassword,angelene:otherpassword` |

`ADMIN_USERS` is a comma-separated list of `username:password`. **Give every reviewer their own
login with their own password.** Decisions and reviewer notes are recorded against these
usernames — a shared login makes attribution meaningless and means you can't revoke one person's
access when they leave the committee without locking everyone out.
